...
const loginApi = "https://api.nrms.go.th/user/v1/authenticate";
...
function* signInUserWithEmailPassword({payload}) {
    ...
    try {
        ...
        if (signInUser.message) {
            ...
            localStorage.setItem('uid', signInUser.userId);
            localStorage.setItem('did', signInUser.departmentId);
            localStorage.setItem('gid', signInUser.groupId);
            localStorage.setItem('token', signInUser.token);
            yield put(userSignInSuccess(signInUser));
            console.log("segas" , signInUser)
        }
    } catch (error) {
        yield put(showAuthMessage(error));
    }
